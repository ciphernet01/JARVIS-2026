#!/bin/bash
# A.S.T.R.A OS Boot Configuration
...
╚═╝  ╚═╝╚══════╝   ╚═╝   ╚═╝  ╚═╝╚═╝  ╚═╝
        ...
        Voice-first spatial operating environment
        Agentic OS control and perception stack
...
# Check A.S.T.R.A backend
echo "Initializing A.S.T.R.A core..."
for i in {1..30}; do
    if curl -s http://localhost:8001/api/health -H "X-JARVIS-TOKEN: system-boot" > /dev/null 2>&1; then
        echo "A.S.T.R.A core online"
        break
    fi
    
    if [ $i -eq 30 ]; then
        echo "A.S.T.R.A core is loading (this may take a moment)"
