...
function activate(context) {
    ...
    // Register commands
    context.subscriptions.push(vscode.commands.registerCommand('jarvis.chat', chatCommand), vscode.commands.registerCommand('jarvis.explain', explainCommand), vscode.commands.registerCommand('jarvis.fix', fixCommand), vscode.commands.registerCommand('jarvis.refactor', refactorCommand), vscode.commands.registerCommand('jarvis.generate', generateCommand), vscode.commands.registerCommand('jarvis.complete', completeCommand));
    // Register inline completion provider
    const config = vscode.workspace.getConfiguration('jarvis');
    if (config.get('autoComplete')) {
        const provider = vscode.languages.registerInlineCompletionItemProvider({ pattern: '**' }, new JarvisCompletionProvider());
        context.subscriptions.push(provider);
    }
    // Register sidebar webview
    const sidebarProvider = new JarvisSidebarProvider(context.extensionUri);
    context.subscriptions.push(vscode.window.registerWebviewViewProvider('jarvis.chatView', sidebarProvider));
...
async function chatCommand() {
    ...
    await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'JARVIS is thinking...' }, async () => {
...
async function explainCommand() {
    ...
    await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'JARVIS analyzing code...' }, async () => {
...
async function fixCommand() {
    ...
    await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'JARVIS fixing bugs...' }, async () => {
...
async function refactorCommand() {
    ...
    await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'JARVIS refactoring...' }, async () => {
        const response = await jarvis.action({
...
async function generateCommand() {
    ...
    await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'JARVIS generating code...' }, async () => {
...
async function completeCommand() {
    ...
    await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'JARVIS completing...' }, async () => {
        const response = await jarvis.action({
            action: 'complete',
            code,
            language,
...
class JarvisCompletionProvider {
